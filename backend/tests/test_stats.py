"""Tests for /api/stats/overview."""
import pytest
from tests.conftest import create_test_movie

# The stats endpoint is at /api/stats/overview (not /api/stats/)
# The router is mounted at /api/stats and the route is GET /overview.
STATS_URL = "/api/stats/overview"
...
async def test_stats_empty(client):
    ...
    assert data["total_watchlisted"] == 0
...
async def test_stats_has_expected_keys(client):
    ...
    data = r.json()
    for key in [
        "total_watched", "total_watchlisted", "total_favorites",
        "average_rating", "top_genres", "by_year", "rating_distribution",
    ]:
